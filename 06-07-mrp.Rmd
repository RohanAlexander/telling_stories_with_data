...
post_stratified_estimates <- 
  improved_alp_support %>% 
  tidybayes::add_predicted_draws(newdata = post_strat_census_data) %>% 
...
post_stratified_estimates %>% 
  ...
  labs(y = "Proportion ALP support",
       x = "State") + 
  # geom_point(data = sample_for_australian_polling %>% 
  #              group_by(state, support_alp) %>%
  #              summarise(n = n()) %>% 
  #              group_by(state) %>% 
  #              mutate(prop = n/sum(n)) %>% 
  #              filter(support_alp==1), 
...
## Case study - Forecasting the 2020 US election
...
## Gender: male and female (that is all Nationscape asked about)
...
us_election_model <- 
  rstanarm::stan_glm(vote_biden ~ gender + age_group + stateicp + education_level,
...
# post_stratified_estimates <- 
#   us_election_model %>% 
#   tidybayes::add_predicted_draws(newdata = poststrat_data) %>% 
#   rename(biden_predict = .prediction) %>% 
#   mutate(biden_predict_prop = biden_predict*cell_prop_of_division_total) %>% 
#   group_by(state, .draw) %>% 
