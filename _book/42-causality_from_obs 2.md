...
Using observational data in sophisticated ways is what this chapter is about. How we can nonetheless be comfortable making causal statements, even when we cannot run A/B tests or RCTs. Indeed, in what circumstances may we actually prefer to not run those or to run observational-based approaches in addition to them. We cover three of the major methods that are in popular use these days: difference-in-differences; regression discontinuity; and instrumental variables.


## Directed acyclic graphs

When we are discussing causality it can help to be very specific about what we mean. It is easy to get so caught up in observational data that they start to trick us. It is important to think really hard, and to use all the tools available to us. For instance, in that earlier example, @dagan2021bnt162b2 were able to use experimental data to guide them. Most of the time, we will not be so lucky as to have both experimental data and observational data available to us. But one framework that can help with thinking really hard about our data is the use of directed acyclic graph (DAG). DAGs are a fancy name for a flow diagram and involves drawing arrows and lines between the variables to indicate the relationship between them. Following @citeerik we use `DiagrammeR` [@citeDiagrammeR] to build them here, because the same approaches are useful outside of just DAGs and `DiagrammeR` provides quite a lot of control (Figure \@ref(fig:firstdag)). But, `ggdag` is also useful [@citeggdag].
...
```{=html}
<div id="htmlwidget-7a88dec70854daf799c7" style="width:75%;height:200px;" class="grViz html-widget"></div>
<script type="application/json" data-for="htmlwidget-7a88dec70854daf799c7">{"x":{"diagram":"\ndigraph {\n  graph [ranksep = 0.2]\n  node [shape = plaintext]\n    x\n    y\n  edge [minlen = 2, arrowhead = vee]\n    x->y\n  { rank = same; x; y }\n}\n","config":{"engine":"dot","options":null}},"evals":[],"jsHooks":[]}</script>
```
...
```{=html}
<div id="htmlwidget-116e0e0ca4db84651537" style="width:75%;height:480px;" class="grViz html-widget"></div>
<script type="application/json" data-for="htmlwidget-116e0e0ca4db84651537">{"x":{"diagram":"\ndigraph {\n  graph [ranksep = 0.2]\n  node [shape = plaintext]\n    Apple\n    Banana\n    Carrot\n  edge [minlen = 2, arrowhead = vee]\n    Apple->Banana\n    Apple->Carrot\n    Carrot->Banana\n  { rank = same; Apple; Banana }\n}\n","config":{"engine":"dot","options":null}},"evals":[],"jsHooks":[]}</script>
```
...
```{=html}
<div id="htmlwidget-e7cdf0faa1ffd6d87e58" style="width:75%;height:480px;" class="grViz html-widget"></div>
<script type="application/json" data-for="htmlwidget-e7cdf0faa1ffd6d87e58">{"x":{"diagram":"\ndigraph {\n  graph [ranksep = 0.2]\n  node [shape = plaintext]\n    Apple\n    Banana\n    Carrot\n  edge [minlen = 2, arrowhead = vee]\n    Apple->Banana\n    Apple->Carrot\n    Banana->Carrot\n  { rank = same; Apple; Banana }\n}\n","config":{"engine":"dot","options":null}},"evals":[],"jsHooks":[]}</script>
```
...
If we use `*` in the regression then it automatically includes the separate aspects as well as their interaction. It is the estimate of $\beta_3$ which is of interest.
...
```r
library(here)
#> here() starts at /Users/rohanalexander/Documents/book
library(haven)
library(huxtable)
#> 
...
21. If you are using regression discontinuity, whare are some aspects to be aware of and think really hard about (select all that apply)?
