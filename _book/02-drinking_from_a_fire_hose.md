...
{\centering \includegraphics[width=0.9\linewidth]{/Users/rohanalexander/Documents/book/figures/01-03_r_essentials/02} 

}

\caption{Opening R Studio Cloud for the first time}(\#fig:second)
\end{figure}
...
The basic requirement for the dataset is that it has the name of the seat (sometimes called a 'division' in Australia) and the party of the person elected. So, a quick sketch of the dataset that we would need could look something like Figure \@ref(fig:canadaexampledata). 
...
\caption{Quick sketch of a dataset that could be useful for analysing Australian elections}(\#fig:canadaexampledata)
\end{figure}
...
\caption{Quick sketch of a possible graph of the number of seats won by each party}(\#fig:canadaexampletable)
\end{figure}
...
Then look for a '+' in the top right, and then select 'New gist'. It will look similar to Figure \@ref(fig:githubgistone).

\begin{figure}

{\centering \includegraphics[width=0.95\linewidth]{/Users/rohanalexander/Documents/book/figures/githubgistone} 

}

\caption{New GitHub Gist}(\#fig:githubgistone)
\end{figure}

Then add all the code to that gist, not just the final bit that is giving an error. And give it a meaningful filename that includes '.R' at the end, for instance, 'australian_elections.R'. In Figure \@ref(fig:githubgisttwo), we have incorrect capitalization `library(Tidyverse)`.

\begin{figure}

{\centering \includegraphics[width=0.95\linewidth]{/Users/rohanalexander/Documents/book/figures/githubgisttwo} 

}

\caption{Create a public GitHub Gist to share code}(\#fig:githubgisttwo)
\end{figure}
...
#> 1        179 Adelaide   SA            33019 Steve    GEORGA~
#> 2        197 Aston      VIC           33330 Alan     TUDGE  
#> 3        198 Ballarat   VIC           32326 Catheri~ KING   
#> 4        103 Banks      NSW           33334 David    COLEMAN
#> 5        180 Barker     SA            33042 Tony     PASIN  
#> 6        104 Barton     NSW           32681 Linda    BURNEY 
#> # ... with 2 more variables: PartyNm <chr>, PartyAb <chr>
tail(raw_elections_data)
#> # A tibble: 6 x 8
#>   DivisionID DivisionNm StateAb CandidateID GivenNm  Surname
#>        <dbl> <chr>      <chr>         <dbl> <chr>    <chr>  
#> 1        152 Wentworth  NSW           33218 Dave     SHARMA 
#> 2        153 Werriwa    NSW           32756 Anne Ma~ STANLEY
#> 3        150 Whitlam    NSW           32632 Stephen  JONES  
#> 4        178 Wide Bay   QLD           33200 Llew     O'BRIEN
#> 5        234 Wills      VIC           32350 Peter    KHALIL 
#> 6        316 Wright     QLD           33376 Scott    BUCHHO~
#> # ... with 2 more variables: PartyNm <chr>, PartyAb <chr>
```
...
#>         <dbl> <chr>       <chr>           <dbl> <chr>    
#> 1         179 Adelaide    SA              33019 Steve    
#> 2         197 Aston       VIC             33330 Alan     
#> 3         198 Ballarat    VIC             32326 Catherine
#> 4         103 Banks       NSW             33334 David    
#> 5         180 Barker      SA              33042 Tony     
#> 6         104 Barton      NSW             32681 Linda    
#> # ... with 3 more variables: surname <chr>, party_nm <chr>,
#> #   party_ab <chr>
```
...
#> [3] "Australian Labor Party" "Liberal"               
#> [5] "Liberal"                "Australian Labor Party"
```
...
Our data now matches our plan (Figure \@ref(fig:canadaexampledata)) pretty well. For every electoral division we have the party of the person that won it. 
...
```r
cleaned_elections_data |> 
  ggplot(aes(x = elected_party)) + # aes abbreviates aesthetics and enables us 
  # to specify the x axis variable
  geom_bar()
```

![](02-drinking_from_a_fire_hose_files/figure-latex/unnamed-chunk-23-1.pdf)<!-- --> 
...
![(\#fig:canadanice)Number of seats won, by political party, at the 2019 Australian Federal Election](02-drinking_from_a_fire_hose_files/figure-latex/canadanice-1.pdf) 
...
1. Following @citeBarrett, please write a stack of four or five atomic habits, related to learning data science, that you could implement this week.
...
15. Please consider the following code, add a small mistake to it, and then create a GitHub Gist that contains all of the code, and submit the URL.
...
Then put together an R Markdown document that simulates some data. Add preamble documentation, then load the packages that are needed: `tidyverse`, `janitor`, and `tidyr`. Add numbers for the riding, then use `sample()` to randomly choose one of six options, with replacement, 338 times.
