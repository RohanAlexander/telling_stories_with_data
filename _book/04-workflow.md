...
![(\#fig:doctervisits)Number of illnesses in the past two weeks, based on the 1977--1978 Australian Health Survey](04-workflow_files/figure-latex/doctervisits-1.pdf) 
...
Now that Git is set-up we need to set-up GitHub. We created an account in Chapter \@ref(drinking-from-a-fire-hose). So log into that. We now need to make a new folder (which is called a 'repo' in Git). Look for a '+' in the top right, and then select 'New Repository' (Figure \@ref(fig:githubtwo)).
