...
\NormalTok{get_data <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(i)\{}
...
\NormalTok{  just_page_i <-}\StringTok{ }\NormalTok{stringi}\OperatorTok{::}\KeywordTok{stri_split_lines}\NormalTok{(all_content[[i]])[[}\DecValTok{1}\NormalTok{]] }
  
  \CommentTok{# Grab the name of the location}
\NormalTok{  area <-}\StringTok{ }\NormalTok{just_page_i[}\DecValTok{3}\NormalTok{] }\OperatorTok{%>%}\StringTok{ }\KeywordTok{str_squish}\NormalTok{()}
\NormalTok{  area <-}\StringTok{ }\KeywordTok{str_to_title}\NormalTok{(area)}
  
  \CommentTok{# Grab the type of table}
\NormalTok{  type_of_table <-}\StringTok{ }\NormalTok{just_page_i[}\DecValTok{2}\NormalTok{] }\OperatorTok{%>%}\StringTok{ }\KeywordTok{str_squish}\NormalTok{()}
  
  \CommentTok{# Get rid of the top matter}
\NormalTok{  just_page_i_no_header <-}\StringTok{ }\NormalTok{just_page_i[}\DecValTok{5}\OperatorTok{:}\KeywordTok{length}\NormalTok{(just_page_i)] }\CommentTok{# Just manually for now, but could create some rules if needed}
  
  \CommentTok{# Get rid of the bottom matter}
\NormalTok{  just_page_i_no_header_no_footer <-}\StringTok{ }\NormalTok{just_page_i_no_header[}\DecValTok{1}\OperatorTok{:}\DecValTok{62}\NormalTok{] }\CommentTok{# Just manually for now, but could create some rules if needed}
  
  \CommentTok{# Convert into a tibble}
\NormalTok{  demography_data <-}\StringTok{ }\KeywordTok{tibble}\NormalTok{(}\DataTypeTok{all =}\NormalTok{ just_page_i_no_header_no_footer)}
  
  \CommentTok{# # Split columns}
\NormalTok{  demography_data <-}
\StringTok{    }\NormalTok{demography_data }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{all =} \KeywordTok{str_squish}\NormalTok{(all)) }\OperatorTok{%>%}\StringTok{ }\CommentTok{# Any space more than two spaces is squished down to one}
\StringTok{    }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{all =} \KeywordTok{str_replace}\NormalTok{(all, }\StringTok{"10 -14"}\NormalTok{, }\StringTok{"10-14"}\NormalTok{)) }\OperatorTok{%>%}\StringTok{ }
\StringTok{    }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{all =} \KeywordTok{str_replace}\NormalTok{(all, }\StringTok{"Not Stated"}\NormalTok{, }\StringTok{"NotStated"}\NormalTok{)) }\OperatorTok{%>%}\StringTok{ }\CommentTok{# Any space more than two spaces is squished down to one}
\StringTok{    }\KeywordTok{separate}\NormalTok{(}\DataTypeTok{col =}\NormalTok{ all,}
             \DataTypeTok{into =} \KeywordTok{c}\NormalTok{(}\StringTok{"age"}\NormalTok{, }\StringTok{"male"}\NormalTok{, }\StringTok{"female"}\NormalTok{, }\StringTok{"total"}\NormalTok{, }\StringTok{"age_2"}\NormalTok{, }\StringTok{"male_2"}\NormalTok{, }\StringTok{"female_2"}\NormalTok{, }\StringTok{"total_2"}\NormalTok{),}
             \DataTypeTok{sep =} \StringTok{" "}\NormalTok{, }\CommentTok{# Just looking for a space. Seems to work fine because the tables are pretty nicely laid out}
             \DataTypeTok{remove =} \OtherTok{TRUE}\NormalTok{,}
             \DataTypeTok{fill =} \StringTok{"right"}
\NormalTok{    )}
  
  \CommentTok{# They are side by side at the moment, need to append to bottom}
\NormalTok{  demography_data_long <-}
\StringTok{    }\KeywordTok{rbind}\NormalTok{(demography_data }\OperatorTok{%>%}\StringTok{ }\KeywordTok{select}\NormalTok{(age, male, female, total),}
\NormalTok{          demography_data }\OperatorTok{%>%}
\StringTok{            }\KeywordTok{select}\NormalTok{(age_}\DecValTok{2}\NormalTok{, male_}\DecValTok{2}\NormalTok{, female_}\DecValTok{2}\NormalTok{, total_}\DecValTok{2}\NormalTok{) }\OperatorTok{%>%}
\StringTok{            }\KeywordTok{rename}\NormalTok{(}\DataTypeTok{age =}\NormalTok{ age_}\DecValTok{2}\NormalTok{, }\DataTypeTok{male =}\NormalTok{ male_}\DecValTok{2}\NormalTok{, }\DataTypeTok{female =}\NormalTok{ female_}\DecValTok{2}\NormalTok{, }\DataTypeTok{total =}\NormalTok{ total_}\DecValTok{2}\NormalTok{)}
\NormalTok{    )}
  
  \CommentTok{# There is one row of NAs, so remove it}
\NormalTok{  demography_data_long <-}\StringTok{ }
\StringTok{    }\NormalTok{demography_data_long }\OperatorTok{%>%}\StringTok{ }
\StringTok{    }\NormalTok{janitor}\OperatorTok{::}\KeywordTok{remove_empty}\NormalTok{(}\DataTypeTok{which =} \KeywordTok{c}\NormalTok{(}\StringTok{"rows"}\NormalTok{))}
...
\NormalTok{all_tables }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{select}\NormalTok{(male, female, total) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate_all}\NormalTok{(}\OperatorTok{~}\KeywordTok{str_remove_all}\NormalTok{(., }\StringTok{"[:digit:]"}\NormalTok{)) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{mutate_all}\NormalTok{(}\OperatorTok{~}\KeywordTok{str_remove_all}\NormalTok{(., }\StringTok{","}\NormalTok{)) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate_all}\NormalTok{(}\OperatorTok{~}\KeywordTok{str_remove_all}\NormalTok{(., }\StringTok{"_"}\NormalTok{)) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate_all}\NormalTok{(}\OperatorTok{~}\KeywordTok{str_remove_all}\NormalTok{(., }\StringTok{"-"}\NormalTok{)) }\OperatorTok{%>%}\StringTok{ }
...
\NormalTok{all_tables}\OperatorTok{$}\NormalTok{male[all_tables}\OperatorTok{$}\NormalTok{male }\OperatorTok{==}\StringTok{ "23-Jun"}\NormalTok{] <-}\StringTok{ }\DecValTok{4923}
\NormalTok{all_tables}\OperatorTok{$}\NormalTok{male[all_tables}\OperatorTok{$}\NormalTok{male }\OperatorTok{==}\StringTok{ "15-Aug"}\NormalTok{] <-}\StringTok{ }\DecValTok{4611}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\KeywordTok{mutate_at}\NormalTok{(}\KeywordTok{vars}\NormalTok{(male, female, total), }\OperatorTok{~}\KeywordTok{str_remove_all}\NormalTok{(., }\StringTok{","}\NormalTok{)) }\OperatorTok{%>%}\StringTok{ }\CommentTok{# First get rid of commas}
\StringTok{  }\KeywordTok{mutate_at}\NormalTok{(}\KeywordTok{vars}\NormalTok{(male, female, total), }\OperatorTok{~}\KeywordTok{str_replace}\NormalTok{(., }\StringTok{"_"}\NormalTok{, }\StringTok{"0"}\NormalTok{)) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate_at}\NormalTok{(}\KeywordTok{vars}\NormalTok{(male, female, total), }\OperatorTok{~}\KeywordTok{str_replace}\NormalTok{(., }\StringTok{"-"}\NormalTok{, }\StringTok{"0"}\NormalTok{)) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate_at}\NormalTok{(}\KeywordTok{vars}\NormalTok{(male, female, total), }\OperatorTok{~}\KeywordTok{as.integer}\NormalTok{(.))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{all_tables}\OperatorTok{$}\NormalTok{area[all_tables}\OperatorTok{$}\NormalTok{area }\OperatorTok{==}\StringTok{ "Taita/ Taveta"}\NormalTok{] <-}\StringTok{ "Taita/Taveta"}
\NormalTok{all_tables}\OperatorTok{$}\NormalTok{area[all_tables}\OperatorTok{$}\NormalTok{area }\OperatorTok{==}\StringTok{ "Elgeyo/ Marakwet"}\NormalTok{] <-}\StringTok{ "Elgeyo/Marakwet"}
\NormalTok{all_tables}\OperatorTok{$}\NormalTok{area[all_tables}\OperatorTok{$}\NormalTok{area }\OperatorTok{==}\StringTok{ "Nairobi City"}\NormalTok{] <-}\StringTok{ "Nairobi"}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{all_tables}\OperatorTok{$}\NormalTok{table }\OperatorTok{%>%}\StringTok{ }\KeywordTok{table}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{list_counties <-}\StringTok{ }
\StringTok{  }\NormalTok{all_tables }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(table }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Table 2.4a: Distribution of Rural Population by Age, Sex* and County"}\NormalTok{,}
...
\NormalTok{all_tables }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(table }\OperatorTok{==}\StringTok{ "Table 2.3: Distribution of Population by Age, Sex*, County and Sub- County"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(area }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Busia"}\NormalTok{,}
...
\NormalTok{         ) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{select}\NormalTok{(area, page) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{distinct}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\NormalTok{all_tables }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{area_type =} \KeywordTok{if_else}\NormalTok{(area }\OperatorTok{%in%}\StringTok{ }\NormalTok{list_counties}\OperatorTok{$}\NormalTok{area, }\StringTok{"county"}\NormalTok{, }\StringTok{"sub-county"}\NormalTok{))}
...
\NormalTok{    area }\OperatorTok{==}\StringTok{ "Busia"} \OperatorTok{&}\StringTok{ }\NormalTok{page }\OperatorTok{==}\StringTok{ }\DecValTok{353} \OperatorTok{~}\StringTok{ "sub-county"}\NormalTok{,}
\NormalTok{    area }\OperatorTok{==}\StringTok{ "Siaya"} \OperatorTok{&}\StringTok{ }\NormalTok{page }\OperatorTok{==}\StringTok{ }\DecValTok{360} \OperatorTok{~}\StringTok{ "sub-county"}\NormalTok{,}
\NormalTok{    area }\OperatorTok{==}\StringTok{ "Homa Bay"} \OperatorTok{&}\StringTok{ }\NormalTok{page }\OperatorTok{==}\StringTok{ }\DecValTok{375} \OperatorTok{~}\StringTok{ "sub-county"}\NormalTok{,}
    \OtherTok{TRUE} \OperatorTok{~}\StringTok{ }\NormalTok{area_type}
\NormalTok{    )}
\NormalTok{  )}

\KeywordTok{rm}\NormalTok{(list_counties)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{table}\NormalTok{(all_tables}\OperatorTok{$}\NormalTok{age) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{head}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{unique}\NormalTok{(all_tables}\OperatorTok{$}\NormalTok{age) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{head}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{all_tables}\OperatorTok{$}\NormalTok{age[all_tables}\OperatorTok{$}\NormalTok{age }\OperatorTok{==}\StringTok{ "NotStated"}\NormalTok{] <-}\StringTok{ "Not Stated"}
\NormalTok{all_tables}\OperatorTok{$}\NormalTok{age[all_tables}\OperatorTok{$}\NormalTok{age }\OperatorTok{==}\StringTok{ "43594"}\NormalTok{] <-}\StringTok{ "5-9"}
\NormalTok{all_tables}\OperatorTok{$}\NormalTok{age[all_tables}\OperatorTok{$}\NormalTok{age }\OperatorTok{==}\StringTok{ "43752"}\NormalTok{] <-}\StringTok{ "10-14"}
\NormalTok{all_tables}\OperatorTok{$}\NormalTok{age[all_tables}\OperatorTok{$}\NormalTok{age }\OperatorTok{==}\StringTok{ "9-14"}\NormalTok{] <-}\StringTok{ "5-9"}
\NormalTok{all_tables}\OperatorTok{$}\NormalTok{age[all_tables}\OperatorTok{$}\NormalTok{age }\OperatorTok{==}\StringTok{ "10-19"}\NormalTok{] <-}\StringTok{ "10-14"}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Add a flag as to whether it's a summary or not}
\NormalTok{all_tables}\OperatorTok{$}\NormalTok{age_type <-}\StringTok{ }\KeywordTok{if_else}\NormalTok{(}\KeywordTok{str_detect}\NormalTok{(all_tables}\OperatorTok{$}\NormalTok{age, }\KeywordTok{c}\NormalTok{(}\StringTok{"-"}\NormalTok{)), }\StringTok{"age-group"}\NormalTok{, }\StringTok{"single-year"}\NormalTok{)}
\NormalTok{all_tables}\OperatorTok{$}\NormalTok{age_type <-}\StringTok{ }\KeywordTok{if_else}\NormalTok{(}\KeywordTok{str_detect}\NormalTok{(all_tables}\OperatorTok{$}\NormalTok{age, }\KeywordTok{c}\NormalTok{(}\StringTok{"Total"}\NormalTok{)), }\StringTok{"age-group"}\NormalTok{, all_tables}\OperatorTok{$}\NormalTok{age_type)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{all_tables}\OperatorTok{$}\NormalTok{age <-}\StringTok{ }\KeywordTok{as_factor}\NormalTok{(all_tables}\OperatorTok{$}\NormalTok{age)}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\NormalTok{all_tables }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{check_sum =}\NormalTok{ male }\OperatorTok{+}\StringTok{ }\NormalTok{female,}
         \DataTypeTok{totals_match =} \KeywordTok{if_else}\NormalTok{(total }\OperatorTok{==}\StringTok{ }\NormalTok{check_sum, }\DecValTok{1}\NormalTok{, }\DecValTok{0}\NormalTok{)}
\NormalTok{         ) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(totals_match }\OperatorTok{==}\StringTok{ }\DecValTok{0}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{all_tables}\OperatorTok{$}\NormalTok{male[all_tables}\OperatorTok{$}\NormalTok{age }\OperatorTok{==}\StringTok{ "10"} \OperatorTok{&}\StringTok{ }\NormalTok{all_tables}\OperatorTok{$}\NormalTok{page }\OperatorTok{==}\StringTok{ }\DecValTok{187}\NormalTok{] <-}\StringTok{ }\KeywordTok{as.integer}\NormalTok{(}\DecValTok{1}\NormalTok{)}
...
\NormalTok{table_}\DecValTok{2}\NormalTok{_}\DecValTok{3}\NormalTok{ <-}\StringTok{ }\NormalTok{all_tables }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(table }\OperatorTok{==}\StringTok{ "Table 2.3: Distribution of Population by Age, Sex*, County and Sub- County"}\NormalTok{)}
\NormalTok{table_}\DecValTok{2}\NormalTok{_4a <-}\StringTok{ }\NormalTok{all_tables }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(table }\OperatorTok{==}\StringTok{ "Table 2.4a: Distribution of Rural Population by Age, Sex* and County"}\NormalTok{)}
\NormalTok{table_}\DecValTok{2}\NormalTok{_4b <-}\StringTok{ }\NormalTok{all_tables }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(table }\OperatorTok{==}\StringTok{ "Table 2.4b: Distribution of Urban Population by Age, Sex* and County"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{both_}\DecValTok{2}\NormalTok{_4s <-}\StringTok{ }\KeywordTok{full_join}\NormalTok{(table_}\DecValTok{2}\NormalTok{_4a, table_}\DecValTok{2}\NormalTok{_4b, }\DataTypeTok{by =} \KeywordTok{c}\NormalTok{(}\StringTok{"age"}\NormalTok{, }\StringTok{"area"}\NormalTok{, }\StringTok{"area_type"}\NormalTok{), }\DataTypeTok{suffix =} \KeywordTok{c}\NormalTok{(}\StringTok{"_rural"}\NormalTok{, }\StringTok{"_urban"}\NormalTok{))}

\NormalTok{all <-}\StringTok{ }\KeywordTok{full_join}\NormalTok{(table_}\DecValTok{2}\NormalTok{_}\DecValTok{3}\NormalTok{, both_}\DecValTok{2}\NormalTok{_4s, }\DataTypeTok{by =} \KeywordTok{c}\NormalTok{(}\StringTok{"age"}\NormalTok{, }\StringTok{"area"}\NormalTok{, }\StringTok{"area_type"}\NormalTok{), }\DataTypeTok{suffix =} \KeywordTok{c}\NormalTok{(}\StringTok{"_all"}\NormalTok{, }\StringTok{"_"}\NormalTok{))}

\NormalTok{all <-}\StringTok{ }
\StringTok{  }\NormalTok{all }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{page =}\NormalTok{ glue}\OperatorTok{::}\KeywordTok{glue}\NormalTok{(}\StringTok{'Total from p. \{page\}, rural from p. \{page_rural\}, urban from p. \{page_urban\}'}\NormalTok{)) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{select}\NormalTok{(}\OperatorTok{-}\NormalTok{page, }\OperatorTok{-}\NormalTok{page_rural, }\OperatorTok{-}\NormalTok{page_urban,}
         \OperatorTok{-}\NormalTok{table, }\OperatorTok{-}\NormalTok{table_rural, }\OperatorTok{-}\NormalTok{table_urban,}
         \OperatorTok{-}\NormalTok{age_type_rural, }\OperatorTok{-}\NormalTok{age_type_urban}
\NormalTok{         )}

\KeywordTok{rm}\NormalTok{(both_}\DecValTok{2}\NormalTok{_4s, table_}\DecValTok{2}\NormalTok{_}\DecValTok{3}\NormalTok{, table_}\DecValTok{2}\NormalTok{_4a, table_}\DecValTok{2}\NormalTok{_4b)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{follow_up <-}\StringTok{ }
\StringTok{  }\NormalTok{all }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{total_from_bits =}\NormalTok{ total_rural }\OperatorTok{+}\StringTok{ }\NormalTok{total_urban,}
         \DataTypeTok{check_total_is_rural_plus_urban =} \KeywordTok{if_else}\NormalTok{(total }\OperatorTok{==}\StringTok{ }\NormalTok{total_from_bits, }\DecValTok{1}\NormalTok{, }\DecValTok{0}\NormalTok{),}
\NormalTok{         total_from_bits }\OperatorTok{-}\StringTok{ }\NormalTok{total) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(check_total_is_rural_plus_urban }\OperatorTok{==}\StringTok{ }\DecValTok{0}\NormalTok{)}

\KeywordTok{head}\NormalTok{(follow_up)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{follow_up <-}\StringTok{ }
\StringTok{  }\NormalTok{all }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{groups =} \KeywordTok{case_when}\NormalTok{(age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"0"}\NormalTok{, }\StringTok{"1"}\NormalTok{, }\StringTok{"2"}\NormalTok{, }\StringTok{"3"}\NormalTok{, }\StringTok{"4"}\NormalTok{, }\StringTok{"0-4"}\NormalTok{) }\OperatorTok{~}\StringTok{ "0-4"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"5"}\NormalTok{, }\StringTok{"6"}\NormalTok{, }\StringTok{"7"}\NormalTok{, }\StringTok{"8"}\NormalTok{, }\StringTok{"9"}\NormalTok{, }\StringTok{"5-9"}\NormalTok{) }\OperatorTok{~}\StringTok{ "5-9"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"10"}\NormalTok{, }\StringTok{"11"}\NormalTok{, }\StringTok{"12"}\NormalTok{, }\StringTok{"13"}\NormalTok{, }\StringTok{"14"}\NormalTok{, }\StringTok{"10-14"}\NormalTok{) }\OperatorTok{~}\StringTok{ "10-14"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"15"}\NormalTok{, }\StringTok{"16"}\NormalTok{, }\StringTok{"17"}\NormalTok{, }\StringTok{"18"}\NormalTok{, }\StringTok{"19"}\NormalTok{, }\StringTok{"15-19"}\NormalTok{) }\OperatorTok{~}\StringTok{ "15-19"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"20"}\NormalTok{, }\StringTok{"21"}\NormalTok{, }\StringTok{"22"}\NormalTok{, }\StringTok{"23"}\NormalTok{, }\StringTok{"24"}\NormalTok{, }\StringTok{"20-24"}\NormalTok{) }\OperatorTok{~}\StringTok{ "20-24"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"25"}\NormalTok{, }\StringTok{"26"}\NormalTok{, }\StringTok{"27"}\NormalTok{, }\StringTok{"28"}\NormalTok{, }\StringTok{"29"}\NormalTok{, }\StringTok{"25-29"}\NormalTok{) }\OperatorTok{~}\StringTok{ "25-29"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"30"}\NormalTok{, }\StringTok{"31"}\NormalTok{, }\StringTok{"32"}\NormalTok{, }\StringTok{"33"}\NormalTok{, }\StringTok{"34"}\NormalTok{, }\StringTok{"30-34"}\NormalTok{) }\OperatorTok{~}\StringTok{ "30-34"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"35"}\NormalTok{, }\StringTok{"36"}\NormalTok{, }\StringTok{"37"}\NormalTok{, }\StringTok{"38"}\NormalTok{, }\StringTok{"39"}\NormalTok{, }\StringTok{"35-39"}\NormalTok{) }\OperatorTok{~}\StringTok{ "35-39"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"40"}\NormalTok{, }\StringTok{"41"}\NormalTok{, }\StringTok{"42"}\NormalTok{, }\StringTok{"43"}\NormalTok{, }\StringTok{"44"}\NormalTok{, }\StringTok{"40-44"}\NormalTok{) }\OperatorTok{~}\StringTok{ "40-44"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"45"}\NormalTok{, }\StringTok{"46"}\NormalTok{, }\StringTok{"47"}\NormalTok{, }\StringTok{"48"}\NormalTok{, }\StringTok{"49"}\NormalTok{, }\StringTok{"45-49"}\NormalTok{) }\OperatorTok{~}\StringTok{ "45-49"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"50"}\NormalTok{, }\StringTok{"51"}\NormalTok{, }\StringTok{"52"}\NormalTok{, }\StringTok{"53"}\NormalTok{, }\StringTok{"54"}\NormalTok{, }\StringTok{"50-54"}\NormalTok{) }\OperatorTok{~}\StringTok{ "50-54"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"55"}\NormalTok{, }\StringTok{"56"}\NormalTok{, }\StringTok{"57"}\NormalTok{, }\StringTok{"58"}\NormalTok{, }\StringTok{"59"}\NormalTok{, }\StringTok{"55-59"}\NormalTok{) }\OperatorTok{~}\StringTok{ "55-59"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"60"}\NormalTok{, }\StringTok{"61"}\NormalTok{, }\StringTok{"62"}\NormalTok{, }\StringTok{"63"}\NormalTok{, }\StringTok{"64"}\NormalTok{, }\StringTok{"60-64"}\NormalTok{) }\OperatorTok{~}\StringTok{ "60-64"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"65"}\NormalTok{, }\StringTok{"66"}\NormalTok{, }\StringTok{"67"}\NormalTok{, }\StringTok{"68"}\NormalTok{, }\StringTok{"69"}\NormalTok{, }\StringTok{"65-69"}\NormalTok{) }\OperatorTok{~}\StringTok{ "65-69"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"70"}\NormalTok{, }\StringTok{"71"}\NormalTok{, }\StringTok{"72"}\NormalTok{, }\StringTok{"73"}\NormalTok{, }\StringTok{"74"}\NormalTok{, }\StringTok{"70-74"}\NormalTok{) }\OperatorTok{~}\StringTok{ "70-74"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"75"}\NormalTok{, }\StringTok{"76"}\NormalTok{, }\StringTok{"77"}\NormalTok{, }\StringTok{"78"}\NormalTok{, }\StringTok{"79"}\NormalTok{, }\StringTok{"75-79"}\NormalTok{) }\OperatorTok{~}\StringTok{ "75-79"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"80"}\NormalTok{, }\StringTok{"81"}\NormalTok{, }\StringTok{"82"}\NormalTok{, }\StringTok{"83"}\NormalTok{, }\StringTok{"84"}\NormalTok{, }\StringTok{"80-84"}\NormalTok{) }\OperatorTok{~}\StringTok{ "80-84"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"85"}\NormalTok{, }\StringTok{"86"}\NormalTok{, }\StringTok{"87"}\NormalTok{, }\StringTok{"88"}\NormalTok{, }\StringTok{"89"}\NormalTok{, }\StringTok{"85-89"}\NormalTok{) }\OperatorTok{~}\StringTok{ "85-89"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"90"}\NormalTok{, }\StringTok{"91"}\NormalTok{, }\StringTok{"92"}\NormalTok{, }\StringTok{"93"}\NormalTok{, }\StringTok{"94"}\NormalTok{, }\StringTok{"90-94"}\NormalTok{) }\OperatorTok{~}\StringTok{ "90-94"}\NormalTok{,}
\NormalTok{                            age }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"95"}\NormalTok{, }\StringTok{"96"}\NormalTok{, }\StringTok{"97"}\NormalTok{, }\StringTok{"98"}\NormalTok{, }\StringTok{"99"}\NormalTok{, }\StringTok{"95-99"}\NormalTok{) }\OperatorTok{~}\StringTok{ "95-99"}\NormalTok{,}
                            \OtherTok{TRUE} \OperatorTok{~}\StringTok{ "Other"}\NormalTok{)}
\NormalTok{         ) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{group_by}\NormalTok{(area_type, area, groups) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{group_sum =} \KeywordTok{sum}\NormalTok{(total, }\DataTypeTok{na.rm =} \OtherTok{FALSE}\NormalTok{),}
         \DataTypeTok{group_sum =}\NormalTok{ group_sum }\OperatorTok{/}\StringTok{ }\DecValTok{2}\NormalTok{,}
         \DataTypeTok{difference =}\NormalTok{ total }\OperatorTok{-}\StringTok{ }\NormalTok{group_sum) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{ungroup}\NormalTok{() }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(age }\OperatorTok{==}\StringTok{ }\NormalTok{groups) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(total }\OperatorTok{!=}\StringTok{ }\NormalTok{group_sum) }
...
\NormalTok{all <-}\StringTok{ }
\StringTok{  }\NormalTok{all }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{rename}\NormalTok{(}\DataTypeTok{male_total =}\NormalTok{ male,}
         \DataTypeTok{female_total =}\NormalTok{ female,}
         \DataTypeTok{total_total =}\NormalTok{ total) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{pivot_longer}\NormalTok{(}\DataTypeTok{cols =} \KeywordTok{c}\NormalTok{(male_total, female_total, total_total, male_rural, female_rural, total_rural, male_urban, female_urban, total_urban),}
               \DataTypeTok{names_to =} \StringTok{"type"}\NormalTok{,}
               \DataTypeTok{values_to =} \StringTok{"number"}
\NormalTok{               ) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{separate}\NormalTok{(}\DataTypeTok{col =}\NormalTok{ type, }\DataTypeTok{into =} \KeywordTok{c}\NormalTok{(}\StringTok{"gender"}\NormalTok{, }\StringTok{"part_of_area"}\NormalTok{), }\DataTypeTok{sep =} \StringTok{"_"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{select}\NormalTok{(area, area_type, part_of_area, age, age_type, gender, number)}

\KeywordTok{write_csv}\NormalTok{(all, }\DataTypeTok{path =} \StringTok{"outputs/data/cleaned_kenya_2019_census.csv"}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{monicas_dataset <-}\StringTok{ }
\StringTok{  }\NormalTok{all }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(area_type }\OperatorTok{==}\StringTok{ "county"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(part_of_area }\OperatorTok{==}\StringTok{ "total"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{filter}\NormalTok{(age_type }\OperatorTok{==}\StringTok{ "single-year"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{select}\NormalTok{(area, age, gender, number)}

\KeywordTok{head}\NormalTok{(monicas_dataset)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{monicas_dataset }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(area }\OperatorTok{==}\StringTok{ "Nairobi"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{ggplot}\NormalTok{() }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_col}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ age, }\DataTypeTok{y =}\NormalTok{ number, }\DataTypeTok{fill =}\NormalTok{ gender), }\DataTypeTok{position =} \StringTok{"dodge"}\NormalTok{) }\OperatorTok{+}\StringTok{ }
...
\StringTok{  }\KeywordTok{scale_x_discrete}\NormalTok{(}\DataTypeTok{breaks =} \KeywordTok{c}\NormalTok{(}\KeywordTok{seq}\NormalTok{(}\DataTypeTok{from =} \DecValTok{0}\NormalTok{, }\DataTypeTok{to =} \DecValTok{99}\NormalTok{, }\DataTypeTok{by =} \DecValTok{5}\NormalTok{), }\StringTok{"100+"}\NormalTok{)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{theme_classic}\NormalTok{()}\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_fill_brewer}\NormalTok{(}\DataTypeTok{palette =} \StringTok{"Set1"}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{y =} \StringTok{"Number"}\NormalTok{,}
       \DataTypeTok{x =} \StringTok{"Age"}\NormalTok{,}
       \DataTypeTok{fill =} \StringTok{"Gender"}\NormalTok{,}
       \DataTypeTok{title =} \StringTok{"Distribution of age and gender in Nairobi in 2019"}\NormalTok{,}
       \DataTypeTok{caption =} \StringTok{"Data source: 2019 Kenya Census"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
