...
- James, Gareth, Daniela Witten, Trevor Hastie and Robert Tibshirani, 2017, *An Introduction to Statistical Learning with Applications in R*, Chapter 3, https://www.statlearning.com.
...
<!-- https://twitter.com/mijkenijk/status/1234899588311474176?s=21 -->
...
We are saying this is a linear regression because we assume that if $x$ doubles then $y$ would also double.
...
- `tidy`: Gives the coefficient estimates in a tidy output.
- `glance`: Gives the diagnostics.
- `augment`: Adds the forecasted values, and hence, residuals, to your dataset.
...
Finally, after creating beautiful graphs and tables you may want your regression output to look just as nice. There are a variety of packages in R that will automatically format your regression outputs. One that is particuarly nice is `huxtable` [@citehuxtable].
...
`Tidymodels` [@citeTidymodels] is what all the cool kids are using these days. It's an attempt to bring some order to the chaos that has been different modelling packages in R. (There have been other attempts in the past and they've crashed and burned, but hopefully this time is different.) The issue is that lets say you want to run a simple linear regression and then run a random forest. The language that you'd use to code these models is fairly different. `Tidymodels` is the latest attempt to bring a coherent grammar to this. It's also a package of packages.
...
<!-- - James, Gareth, Daniela Witten, Trevor Hastie and Robert Tibshirani, 2017, 'An Introduction to Statistical Learning with Applications in R' Chapter 4. -->
...
<!--  López de Prado, Marcos, Clustering (Presentation Slides) (January 2, 2020). Available at SSRN: https://ssrn.com/abstract=3512998 or http://dx.doi.org/10.2139/ssrn.3512998  -->



<!-- https://francoishusson.wordpress.com/2020/03/12/all-you-need-to-know-on-clustering-with-factoshiny/ -->
